...
const activateDelete = destElem => {
  const targets = destElem ? destElem.find('[deleteurl]') : $('[deleteurl]')
  targets.each((i, elem) => {
    const el = $(elem)
    const deleteUrl = el.attr('deleteurl')
    const msg = `Are you sure to ${el.attr('title')}?`
